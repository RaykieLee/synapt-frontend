...
export const dictAPI = {
  ...
  getDictTypes: (params: DictTypeQuery) => {
    ...
    return apiRequest<PaginationResult<DictType>>('/api/v1/system/dicts/types/list', 'POST', requestBody);
  },

  /**
   * 获取字典类型详情
   */
  getDictTypeDetail: (dictId: number) => apiRequest<DictType>(`/api/v1/system/dicts/types/${dictId}`, 'GET'),

  /**
   * 创建字典类型
   */
  createDictType: (data: DictTypeCreateDto) => apiRequest<DictType>('/api/v1/system/dicts/types', 'POST', data),

  /**
   * 更新字典类型
   */
  updateDictType: (dictId: number, data: DictTypeUpdateDto) => apiRequest<DictType>(`/api/v1/system/dicts/types/${dictId}`, 'PUT', data),
   ...
   * 获取字典数据详情
   */
  getDictDataDetail: (dictCode: number) => apiRequest<DictData>(`/api/v1/system/dicts/data/${dictCode}`, 'GET'),

  /**
   * 创建字典数据
   */
  createDictData: (data: DictDataCreateDto) => apiRequest<DictData>('/api/v1/system/dicts/data', 'POST', data),

  /**
   * 更新字典数据
   */
  updateDictData: (dictCode: number, data: DictDataUpdateDto) => apiRequest<DictData>(`/api/v1/system/dicts/data/${dictCode}`, 'PUT', data),

  /**
   * 删除字典数据
   */
  deleteDictData: (dictCode: number) => apiRequest<void>(`/api/v1/system/dicts/data/${dictCode}`, 'DELETE'),

  /**
   * 根据字典类型获取字典选项
   */
  getDictOptions: (dictType: string) => apiRequest<DictOption[]>(`/api/v1/system/dicts/type/${dictType}`, 'GET')
};
...
export const dictApi = {
  getList: (params: DictQuery) => {
    ...
    return apiRequest<Dict>('/api/v1/system/dict', 'POST', requestBody);
  },
