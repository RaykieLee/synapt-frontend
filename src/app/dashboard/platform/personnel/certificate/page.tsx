...
import { CertificateQuery, Certificate } from "@/types/personnel";
...
export default function CertificatePage() {
  // 状态管理
  const [query, setQuery] = useState<CertificateQuery>({
    page_num: 1,
    page_size: 10,
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ["personnel", "certificate", "list", query],
    queryFn: () => certificateAPI.getList(query),
  });

  const data = (response?.data || response || {}) as PageResult<Certificate>;
